...
Run script version for faster execution
"""
...
n = 2000  # Reduced from 5000 for faster Volterra computation
...
def analyze_and_scale_spectrum(A, r, n):
    ...
    # Scale A so mean(lambda) approx 1
...
def run_experiment_sweep(A, b, x_star, eigvals, gamma_max, r, n, num_epochs, num_runs, sweep_name=""):
    ...
    for i, mult in enumerate(multipliers):
        ...
        psi, t_theory = solver.solve(t_max=num_epochs, dt=0.1)  # Increased from 0.05 for speed
        print(f"Done!")
...
for r in [0.5, 1.0, 1.2]:
    ...
    print(f"\nr = {r}:")
    print(f"  d = {int(n * r)}")
    print(f"  Spectral ratio lambda_max/mean(lambda) = {res['spectral_info']['spectral_ratio']:.2f}")
...
print(f"Results saved in 'all_results' dictionary")
