...
\begin{columns}[T]
    ...
    \begin{column}{0.83\linewidth}
        \begin{beamercolorbox}[wd=\linewidth,ht=3.5cm,dp=0.5cm,center]{palette primary}
            {\Huge \textbf{Testing the Volterra Model for SGD on Real Data}}\\[0.3cm]
            {\Large Kaan Alta\c{s} \& Yi\u{g}it Karasu}\\[0.2cm]
            {\normalsize ELEC/COMP 450: Introduction to Modern Learning Theory \hfill Instructor: Asst. Prof. Zafer Do\u{g}an}
        \end{beamercolorbox}
    \end{column}
\end{columns}
...
\begin{column}{0.48\linewidth}
\centering
\includegraphics[width=\linewidth]{../poster_figures/fig1_replication.png}
\end{column}
\begin{column}{0.48\linewidth}
\centering
\includegraphics[width=\linewidth]{paper_result.png}
\end{column}
\end{columns}

\vspace{0.1cm}
\begin{center}
\small \textit{Left: Our replication. Right: Original paper (Fig. 4, $\gamma = \gamma_{\max}/8$).}
\end{center}

\textbf{Observation:} Volterra (green) accurately predicts SGD dynamics. SDE underestimates due to isotropic noise assumption. Streaming continues decreasing (infinite data).
\end{block}
...
% --- 5. NEW RESULTS ---
\begin{block}{5. New Results: MNIST Random Features}
\justifying

\textbf{(a) Base Setting:} Linear target $\mathbf{b} = A\mathbf{x}^*$ --- Realizable
\vspace{0.1cm}
\begin{center}
\includegraphics[width=0.95\linewidth]{../poster_figures/fig2_base_mnist.png}
\end{center}
\vspace{-0.2cm}
...
\vspace{0.1cm}
\begin{center}
\includegraphics[width=0.95\linewidth]{../poster_figures/fig3_nonlinear_mnist.png}
\end{center}
\vspace{-0.2cm}
...
\vspace{0.1cm}
\begin{center}
\includegraphics[width=0.95\linewidth]{../poster_figures/fig4_whitened_mnist.png}
\end{center}
\vspace{-0.2cm}
...
\vspace{0.1cm}
\begin{center}
\includegraphics[width=0.95\linewidth]{../poster_figures/fig5_criticality.png}
\end{center}
\vspace{-0.2cm}
...
% --- 7. KEY FINDINGS ---
\begin{block}{7. Key Findings \& Critical Analysis}
\justifying
\begin{itemize}
    \item \textbf{Volterra works on real data:} Correlation $R > 0.97$ even for non-linear, non-realizable targets
    ...
    \item \textbf{Critical step size gap:} Theory overestimates safe $\gamma$ by $\sim$100x due to outlier eigenvalues
\end{itemize}
\end{block}

% --- 8. FUTURE DIRECTIONS ---
\begin{block}{8. Future Directions}
\justifying
\begin{itemize}
    \item Extend to \textbf{mini-batch SGD} ($B > 1$) and momentum methods
    \item Test on \textbf{deeper networks} (multi-layer random features)
    \item Develop \textbf{robust $\gamma_{\max}$} using trimmed eigenvalue statistics
    \item Apply to \textbf{neural tangent kernel} (NTK) regime
\end{itemize}
\end{block}
...
\bibitem{paquette2021} Paquette, C., Lee, K., Pedregosa, F., Paquette, E. (2021). \textit{SGD in the Large: Average-case Analysis, Asymptotics, and Stepsize Criticality}. NeurIPS.
\bibitem{bach2024} Bach, F. (2024). \textit{Learning Theory from First Principles}. MIT Press.
\end{thebibliography}
\end{block}
