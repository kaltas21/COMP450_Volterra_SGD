...
This experiment tests Volterra theory when targets are non-linear.
Key difference from run_part2.py: b = ReLU(A @ x_star) instead of b = A @ x_star

This creates a non-realizable problem (no exact solution exists), testing
whether Volterra theory still predicts SGD dynamics accurately.
"""
...
def generate_nonlinear_planted_targets(A, device):
    ...
    Key difference from run_part2.py: ReLU is applied to create non-realizable targets.
    This means there is no x such that Ax = b exactly.
    """
    n, d = A.shape
    ...
    # Track statistics about the non-linearity
    ...
    print(f"\n=== Non-Linear Planted Targets (ReLU) ===")
    print(f"||x*||^2 = {torch.norm(x_star).item()**2:.4f}")
    print(f"Zero fraction in b (clipped by ReLU): {zero_fraction:.2%}")
...
def run_experiment_sweep(A, b, x_star, eigvals, gamma_max, r, n, num_epochs, num_runs, sweep_name=""):
    ...
    for i, mult in enumerate(multipliers):
        ...
        # Volterra Theory (using R=1 as approximation for non-realizable case)
...
print("\n" + "="*60)
print("STARTING NON-LINEAR TARGET EXPERIMENTS")
print("="*60)
...
for idx, r in enumerate(aspect_ratios):
    ...
    # Generate NON-LINEAR planted targets (KEY DIFFERENCE)
    x_star, b, x0, initial_loss, zero_fraction = generate_nonlinear_planted_targets(A, device)
    ...
    # Recompute b after scaling A (apply ReLU again)
    linear_target_scaled = A_scaled @ x_star
    b_scaled = torch.nn.functional.relu(linear_target_scaled)
    initial_loss_scaled = (1.0 / (2 * n)) * torch.sum(b_scaled**2).item()
    zero_fraction_scaled = (b_scaled == 0).float().mean().item()
    print(f"\nAfter scaling: Initial loss f(x0) = {initial_loss_scaled:.4f}")
    print(f"After scaling: Zero fraction = {zero_fraction_scaled:.2%}")
    ...
    all_results[r] = {
        ...
        'zero_fraction': zero_fraction_scaled,  # Extra info for this experiment
...
# Save results
results_dir = './results/part2_nonlinear_target'
os.makedirs(results_dir, exist_ok=True)
np.save(os.path.join(results_dir, 'all_results.npy'), all_results)

# Summary
print("\n" + "="*60)
print("SUMMARY - NON-LINEAR TARGET EXPERIMENT")
print("="*60)
...
for r in aspect_ratios:
    ...
    print(f"  Zero fraction (ReLU clipped) = {res['zero_fraction']:.2%}")
